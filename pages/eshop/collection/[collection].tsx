...
const Category = () => {
  ...
  const [cart, setCart] = useState({}); // State to track quantities for each item
  ...
  useEffect(() => {
    const total = collections.listItemsByCollection.items.reduce(
      (sum, item) => {
        return sum + item.price * (cart[item.itemId] || 0);
      },
      0
    );
    setTotalPrice(total);
